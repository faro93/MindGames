...
class E18():
    def __init__(self, t):
        self.triangle = t
        self.t = list()
        self.Rtriangle = self.triangle[::-1]

    def Info(self):
        print(f'self.triangle={self.triangle}')
        print(f'self.Rtriangle={self.Rtriangle}')

    def AddBottom(self):
        y = int()
        s = list()
        while len(self.Rtriangle)-1:
            # print(f'l1:{self.Rtriangle[0]}\nl2:{self.Rtriangle[1]}')
            for x2 in range(len(self.Rtriangle[1])):
                for x1 in range(x2, x2+2):
                    # print(f'{x1}.{x2} : ', end='')
                    # print(f'{self.Rtriangle[0][x1]}+{self.Rtriangle[1][x2]}')
                    s.append(self.Rtriangle[0][x1]+self.Rtriangle[1][x2])
                # print(f's={s}')
                self.t.append(max(s))
                # print(f'self.t={self.t} ')
                s.clear()
            if y < len(self.Rtriangle)-1:
                self.Rtriangle.pop(1)
            self.Rtriangle.pop(0)
            self.Rtriangle.insert(0, self.t.copy())
            self.t.clear()
            # print(f'self.Rtriangle={self.Rtriangle}')
            # print()
        print(f'La plus grande somme du triangle est {self.Rtriangle[0][0]}.')
